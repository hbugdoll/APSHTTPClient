...
+ (int)caselessCompareFirstString:(const char *)firstString secondString:(const char *)secondString size:(int)size
{
  ...
  while (index < size) {
    ...
    if (firstChar != secondChar)
      return index; //Yes, this is one after the failure.
  }
  return 0;
}

#define TRYENCODING(encodingName, nameSize, returnValue, value)                                                              \
  if ((remainingSize > nameSize) && ([self caselessCompareFirstString:data secondString:encodingName size:nameSize] == 0)) { \
    *value = returnValue;                                                                                                    \
    return YES;                                                                                                              \
  }
...
+ (BOOL)extractEncodingFromData:(NSData *)inputData result:(NSStringEncoding *)result
{
  ...
  int remainingSize = (int)[inputData length];
  int unsearchableSize;
  if (remainingSize > 2008)
    unsearchableSize = remainingSize - 2000;
  else
    unsearchableSize = 8; // So that there's no chance of overrunning the buffer with 'charset='
